JQTWEET = {
	...
	hash:'%23mcreats OR %23manchestereats',
	appendTo: '#jstwitter',
	imgWidth: 220,
		...
		$.ajax({
			...
			success: function(data, textStatus, xhr) {
			    data = data['results'];
                 ...
                 var tweet_box = '<div class="tweet">IMG_TAG';
                 tweet_box+= '<div class="tweet-user">@USER: </div>';
                 tweet_box+= '<span class="tweet-text">TWEET_TEXT</span>';
                 tweet_box+= '<div class="time"><i class="icon-time"></i> <span class="time-text">AGO</span></div>';
				 ...
				 for (var i = 0; i < data.length; i++) {
					try {
						...
						if (data[i].entities.media) {
							...
							img += '<img src="' + data[i].entities.media[0].media_url + ':small" alt=""/>';
							img += '</a>';
						} else {
							img = '';
						}
					...
					$(JQTWEET.appendTo).append(
						tweet_box.replace('IMG_TAG', img)
							.replace('TWEET_TEXT', JQTWEET.ify.clean(data[i].text, img) )
							.replace(/USER/g, '<a href="http://www.twitter.com/' + username + '">' + username + '</a>')
